...
| 3 | Type: *Pump P-204 drive-end vibration is 8.2 mm/s RMS. What zone is that?* — point at **Auto · Reasoning** in the composer before sending | routing is visible before the request runs; retrieval; citation | ~6s, "Zone D" with a **§4.2** pill; hover it to show the clause, click to open it among the sources |
| 4 | Follow up: *And who has to approve it?* | multi-turn memory | Section Head and Head of Maintenance |
| 5 | Attach `Inspection_Report_P-204.pdf` (clip or drag onto the page), ask for findings against the SOP | scanned PDF, no text layer → vision | expand the reasoning line: the page read, tier `LV` |
...
| 9 | Panel toggle (top right) → **Shield** → **Attempt an external call** | the tripwire | `BLOCKED` in ~1ms |
| 9b | Composer model menu → pick **Reasoning**, ask *What is 18 × 47?* | automatic routing is the default, not a cage | chip says *your pick*; the seal still says *numbers computed, not generated*; set it back to **Auto** |
